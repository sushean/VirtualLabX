  ...
  return (
    <div className="min-h-screen text-white relative font-sans flex items-center justify-center pt-48 pb-12 px-6 overflow-hidden">
      ...
      <div className="animate-page-enter w-full max-w-md z-10 transition-all duration-500">
        ...
        <div className="glass-card p-10 md:p-12 relative overflow-hidden backdrop-blur-2xl bg-[var(--panel-bg)] border border-[var(--glass-border)] rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.6)]">
          ...
          {!isFirebaseConfigured && (
            <div className="bg-amber-500/10 border border-amber-500/40 text-amber-300 text-xs p-4 rounded-xl text-center shadow-inner leading-relaxed mb-6">
              <strong>Firebase Setup Required:</strong> To enable Google, GitHub, and email login, please configure your Firebase credentials in the <code>.env</code> file.
            </div>
          )}
          ...
          <form className="space-y-5 animate-page-enter" onSubmit={async (e) => {
             ...
             try {
                if(isLogin) {
                   ...
                   // Log in user on context
                   login(email, password); // We can just log them in using the login method
                   navigate('/');
                }
             } catch(err) {
                setErrorMsg(err.response?.data?.msg || err.message || 'An error occurred during authentication.');
             } finally {
                setLoading(false);
             }
          }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    placeholder="Alan"
                    className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-5 py-4 text-white focus:outline-none focus:border-[#00e5ff]/50 focus:bg-white/10 transition-all font-medium placeholder:text-gray-600 shadow-inner hover:border-white/20"
                  />
                </div>
                <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-5 py-4 text-white focus:outline-none focus:border-[#00e5ff]/50 focus:bg-white/10 transition-all font-medium placeholder:text-gray-600 shadow-inner group-hover:border-white/20"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <input 
                  ...
                  placeholder="••••••••"
                  className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-5 py-4 text-white focus:outline-none focus:border-purple-500/50 focus:bg-white/10 transition-all font-medium placeholder:text-gray-600 shadow-inner group-hover:border-white/20"
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button 
              ...
              className="flex items-center justify-center gap-2 bg-[var(--glass-bg)] hover:bg-white/10 border border-[var(--glass-border)] py-3 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button 
              ...
              disabled={loading || !isFirebaseConfigured}
              className="flex items-center justify-center gap-2 bg-[var(--glass-bg)] hover:bg-white/10 border border-[var(--glass-border)] py-3 rounded-xl transition-all group disabled:opacity-50 disabled:cursor-not-allowed"
            >
