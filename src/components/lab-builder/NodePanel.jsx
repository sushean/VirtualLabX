...
export default function NodePanel() {
  ...
  return (
    <div className="w-64 bg-[var(--panel-bg)] border-r border-[var(--glass-border)] p-4 flex flex-col h-full z-10 shadow-[2px_0_15px_rgba(0,0,0,0.5)] overflow-y-auto custom-scrollbar">
      ...
      <p className="text-xs text-gray-400 mb-6 pb-4 border-b border-[var(--glass-border)]">Drag these nodes onto the canvas.</p>
          ...
          <div key={category}>
             ...
             <div className="flex flex-col gap-2">
                {Object.entries(NodeRegistry).filter(([_, def]) => def.category === category).map(([type, def]) => {
                   let borderColor = 'border-[var(--glass-border)] hover:border-purple-500';
                   ...
                   return (
                     <div
                        key={type}
                        className={`bg-[#120a1f]/80 p-3 rounded-lg border ${borderColor} cursor-grab active:cursor-grabbing text-center text-sm font-bold text-gray-300 transition-all hover:bg-[var(--glass-bg)] hover:shadow-lg shadow-[0_4px_10px_rgba(0,0,0,0.2)]`}
