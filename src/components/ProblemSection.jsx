      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full z-10 overflow-hidden py-4 px-2">
        {problems.map((prob, idx) => (
          <div key={idx} className={`gsap-card-${idx} h-full`}>
            <div className="glass-card p-6 flex flex-col items-center text-center group hover:bg-[var(--panel-bg-strong)] transition-all border border-[var(--glass-border)] hover:border-[var(--accent-cyan)] relative overflow-hidden shadow-lg hover:shadow-[0_0_30px_rgba(0,229,255,0.2)] transform hover:-translate-y-2 duration-300 h-full">
              ...
              <div className="mb-6 relative z-10 w-full h-[220px] flex justify-center items-center overflow-hidden rounded-xl bg-[var(--panel-bg)] border border-[var(--panel-border)] shadow-inner">
              ...
              <h3 className="text-2xl font-bold text-[var(--page-text)] mb-4 relative z-10 group-hover:text-[var(--accent-cyan)] transition-colors">{prob.title}</h3>
              <p className="text-[var(--muted-text)] text-sm leading-relaxed relative z-10 font-medium tracking-wide">
                {prob.desc}
              </p>
            </div>
          </div>
